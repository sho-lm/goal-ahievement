...
RSpec.describe "Sessions", type: :request do
  ...
  describe "POST #create" do
    context "when specified user name is wrong" do
      let(:wrong_name_params) { { session: { name: "not sally name", password: sally.password } } }
      it "returns HTTP STATUS 200" do
        post login_path, params: wrong_name_params
        expect(response.status).to eq 200  
      end
    ...
    context "when specified user password is wrong" do
      let(:wrong_password_params) { { session: { name: sally.name, password: "not sally password"} } }
      it "returns HTTP STATUS 200" do
